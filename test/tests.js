...
var easychain = require("../index.js");

describe("easychain JS", function () {

	it("should be ok", function () {
		(easychain).should.be.ok;
	});

	it("should be object", function () {
		(easychain).should.be.type("object");
	});
	...
	it("should have properties", function () {
		...
		properties.forEach(function (property) {
			(easychain).should.have.property(property);
		});
	});

	describe("crypto.js", function () {
		var crypto = easychain.crypto;
	...
	describe("dapp.js", function () {
		...
		describe("#createDApp", function () {
			...
			var options = {
				"name": "easychain-dapp-cctime",
				"link": "https://github.com/easychainPlatform/easychain-dapp-cctime/archive/master.zip",
				"category": 1,
				"description": "Decentralized news channel",
				"tags": "easychain,dapp,demo,cctime",
			...
			describe("returned dapp", function () {
				var keys = easychain.crypto.getKeys("secret");
				var secondKeys = easychain.crypto.getKeys("secret 2");
				...
				it("should be signed correctly", function () {
					var result = easychain.crypto.verify(trs);
					(result).should.be.ok;
				});
				...
				it("should be second signed correctly", function () {
					trs.amount = 0;
					var result = easychain.crypto.verifySecondSignature(trs, secondKeys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should be ok to verify bytes", function () {
					...
					var secret = 'secret1'
					var keys = easychain.crypto.getKeys(secret)
					var signature = easychain.crypto.signBytes(data1, keys)
					var result = easychain.crypto.verifyBytes(data1, signature, keys.publicKey)
					result.should.be.ok

					var data2 = new Buffer('a1b2c3d4', 'hex')
					signature = easychain.crypto.signBytes(data2, keys)
					result = easychain.crypto.verifyBytes(data2, signature, keys.publicKey)
					result.should.be.ok
				})
			});
		});
	...
	describe("delegate.js", function () {
		var delegate = easychain.delegate;
		...
		describe("#createDelegate", function () {
			...
			describe("returned delegate", function () {
				...
				it("should be signed correctly", function () {
					var result = easychain.crypto.verify(trs, keys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should not be second signed correctly now", function () {
					trs.amount = 100;
					var result = easychain.crypto.verify(trs, secondKeys.publicKey);
					(result).should.be.not.ok;
				});
	...
	describe("signature.js", function () {
		var signature = easychain.signature;
		it("should be ok", function () {
	...
	describe("transaction.js", function () {
		var transaction = easychain.transaction;
		...
		describe("#createTransaction with second secret", function () {
			...
			var keys = easychain.crypto.getKeys(secondSecret);
			...
			describe("returned transaction", function () {
				...
				it("should be second signed correctly", function () {
					var result = easychain.crypto.verifySecondSignature(trs, keys.publicKey);
					(result).should.be.ok;
				});
	...
	describe("vote.js", function () {
		...
		describe("#createVote", function () {
			var createVote = vote.createVote,
				vt = null,
				publicKey = easychain.crypto.getKeys("secret").publicKey,
			...
			describe("returned vote", function () {
				...
				it("should be signed correctly", function () {
					var result = easychain.crypto.verify(vt);
					(result).should.be.ok;
				});

				it("should be second signed correctly", function () {
					var result = easychain.crypto.verifySecondSignature(vt, easychain.crypto.getKeys("second secret").publicKey);
					(result).should.be.ok;
				});
	...
	describe('crypto sha256 and address', function () {
		it('should be equal to the expected address', function () {
			easychain.crypto.getAddress('7a91b9bfc0ea185bf3ade9d264da273f7fe19bf71008210b1d7239c82dd3ad20').should.be.equal('AFbYJhiJb3DXzHy5ZP24mKw21M2dCBJCXP')
			var publicKeyBuffer = new Buffer('7a91b9bfc0ea185bf3ade9d264da273f7fe19bf71008210b1d7239c82dd3ad20', 'hex')
			easychain.crypto.getAddress(publicKeyBuffer).should.be.equal('AFbYJhiJb3DXzHy5ZP24mKw21M2dCBJCXP')
		})
	})
